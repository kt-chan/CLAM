...
with open(
    args.save_dir + "/eval_experiment_{}.txt".format(args.save_exp_code), "w"
) as f:
...
if args.task == "task_1_tumor_vs_normal":
    ...
    dataset = Generic_MIL_Dataset(
        ...
        data_dir=os.path.join(args.data_root_dir, args.data_set_name),
        shuffle=False,
        print_info=True,
...
ckpt_paths = [
    os.path.join(args.models_dir, "s_{}_checkpoint.pt".format(fold)) for fold in folds
]
...
if __name__ == "__main__":
    ...
    for ckpt_idx in range(len(ckpt_paths)):
        ...
        df.to_csv(
            os.path.join(args.save_dir, "fold_{}.csv".format(folds[ckpt_idx])),
            index=False,
        )
